%{
    ...
    quad *qArray[NSYMS];
    int quadPtr = 0;
    int offset = 0; // offset for symbol tables
    char * t = ""; // type of variable
    int w = 0; // width of variable
%}
...
value:    value ',' IDENTIFIER { update($3->name, t,w,offset);
                                    offset += w; }
        // | value ',' assignment_expression
        | IDENTIFIER { update($1->name, t, w, offset);
                        offset += w; }
...
void update(char * name, char *type, int width, int offset){
    struct symtab *sp;
    sp = symlook(name);
    sp->type = type;
    sp->offset = offset;
